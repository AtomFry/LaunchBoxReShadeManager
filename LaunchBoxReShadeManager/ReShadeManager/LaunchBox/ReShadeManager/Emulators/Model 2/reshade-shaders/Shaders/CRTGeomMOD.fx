...
/* 
	CRTGeomMOD v3.2 (2020-16-04) Shader customized and enhanced by Ducon2016 and Houb
...
uniform float2 aspect_ratio <
	...
	ui_category = "Resize Advanced Settings";
> = float2(1.0, 1.0);
...
float2 ExtUV(float2 uv)
{
	...
	if (ROTATED_180)
		ext_uv = float2(1.0f - ext_uv.x, 1.0f - ext_uv.y);
...
float2 BezelUV(float2 uv)
{
	...
	if (!ROTATED)
	{
		if ((video_size.x / video_size.y) <= (screen_size.x / screen_size.y))
		{
			if (game_ratio <= screen_ratio)
...
float2 Asp()
{
	...
	return float2(1.0, 1.0f / game_ratio);
}
